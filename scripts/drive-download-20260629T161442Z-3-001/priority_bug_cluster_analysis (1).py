...
- Core bugs = filled circles, outliers = X markers with bug ID + top labels shown
...
Output:
    - New_priority_bug_cluster_interactive.html
    - New_priority_bug_cluster_static.png
"""
...
try:
    ...
    install("plotly"); import plotly.graph_objects as go
...
try:
    ...
    install("umap-learn"); import umap
...
OUTPUT_HTML = os.path.join(BASE_DIR, "priority_2nd_0_45_bug_cluster_interactive.html")
OUTPUT_PNG = os.path.join(BASE_DIR, "priority_2nd_0_45_bug_cluster_static.png")
...
ELLIPSE_STD   = 1.5
IQR_FACTOR    = 1.5
TOP_LABELS_ON_PLOT = 3   # number of label names to show on static plot per outlier
...
def run():
    ...
    all_unique_labels = list(set(label for bug in bugs for label in bug['labels']))
        ...
        if len(data['x']) >= 4:
            ...
            if n_out > 0:
                for i, is_core in enumerate(mask):
                    if not is_core:
                        top_labels = data['labels'][i][:TOP_LABELS_ON_PLOT]
    ...
    # Summary stats
    print(f"\n  Per-Priority Stats:")
    print(f"  {'Priority':<10} {'N':>4} {'Core':>6} {'Outliers':>9} {'Centroid X':>12} {'Centroid Y':>12} {'Spread':>8}")
    print(f"  {'-'*65}")
    centroids = {}
    for pri in ['P1','P2','P3','P4','P5']:
        data = pri_data[pri]
        if data['x']:
            ...
            centroids[pri] = (cx, cy)
            print(f"  {pri:<10} {len(data['x']):>4} {sum(mask):>6} {sum(~mask):>9} {cx:>12.2f} {cy:>12.2f} {spread:>8.2f}")
    ...
    for pri, data in pri_data.items():
        ...
        if out_x:
            # Show ALL labels in hover for outliers
            hover_out = [
            ...
            fig.add_trace(go.Scatter(
                ...
                marker=dict(color=COLORS[pri], size=14, opacity=0.9,
                           symbol="x", line=dict(color=COLORS[pri], width=2)),
                text=[f"  Bug {bid}" for bid in out_ids],
    ...
    fig.update_layout(
        title=dict(
            text=f"Firefox Bug Cluster Analysis by Priority (Bug-Level)<br>"
                 f"<sup>Hover outliers (✕) to see ALL labels | Core=●  Outlier=✕ | "
    ...
    for pri, data in pri_data.items():
        ...
        if out_x:
            ...
            for i in range(len(out_x)):
                # Show Bug ID + ALL labels on static plot
                all_lbls = out_labels[i]
                label_str = "\n".join([f"• {l}" for l in all_lbls])
                annotation_text = f"Bug {out_ids[i]}\n{label_str}"

                ax.annotate(
                    annotation_text,
                    (out_x[i], out_y[i]),
                    fontsize=5.5, color=COLORS[pri], fontweight='bold',
                    xytext=(10, 10), textcoords="offset points",
                    bbox=dict(boxstyle='round,pad=0.3', facecolor='white',
                             alpha=0.90, edgecolor=COLORS[pri], linewidth=1),
        ...
        ax.annotate(
            ...
            bbox=dict(boxstyle="round,pad=0.4", facecolor=COLORS[pri], edgecolor="white", alpha=0.9)
        )
        # Centroid cross marker
        ax.plot(cx, cy, marker='+', color='white', markersize=10, markeredgewidth=2, zorder=5)

    ax.set_title(
        f"Firefox Bug Cluster Analysis by Priority (Bug-Level)\n"
        f"Core bugs ● | Outliers ✕ with Bug ID + All Labels | Centroid shown as + | "
